...
@if($tarea->task_assitance!=null)
<div class="text-lg font-bold text-center">Hora iniciada: {{$tarea->task_assitance->hora_llegada}}</div>
<div class="text-lg font-bold text-center">Hora finalizada: {{$tarea->task_assitance->hora_termianda}}</div>
@else
    <div class="text-lg font-bold text-center">Tarea aun no iniciada</div>
...
<div class="grid grid-cols-3 justify-items-center border-t-4 border-blue-200 my-15 pt-10">
    ...
    <div class="group  hover:bg-white hover:shadow-lg hover:border-transparent border border-blue-300  font-normal shadow max-w-sm mx-auto rounded-md p-4 bg-white mt-2 mb-5 ">
    ...
    <div class="text-lg font-bold text-center">Cliente
        <div class="group  hover:bg-white hover:shadow-lg hover:border-transparent border border-blue-300  font-normal shadow rounded-md p-4 bg-white mt-2 mb-5 ">
            <a class="flex" href="{{route('clientes.more',['id'=>$tarea->client->id])}}">
            @if($tarea->client!=null)
                    <div class="flex-row ">
                        <div>
                            <div class="text-lg font-bold">{{$tarea->client->nombre}}
                            </div>
                        </div>
                        <div class="mt-3">
                            <p class="text-sm">Correo: {{$tarea->client->correo}}</p>
                            <p class="mt-0.5 text-sm">Teléfono: {{$tarea->client->telefono}}</p>
                            <p class="mt-0.5 text-sm">Dirección: {{$tarea->client->direccion}}</p>
                        </div>
                    </div>
                @endif
            </a>
